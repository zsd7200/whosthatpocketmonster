...
window.onload = function () {
  ...
  var randomImg = function randomImg(imgsArr, imgsPath) {
    ...
    var randMon,
        loop = false,
        formArr = []; // swap visibility of canvas and mobile container
    ...
    } while (loop); // set src of img to random pokemon img
    ...
    img.onload = function () {
      ...
      if (!isMobileRes) ctx.clearRect(0, 0, canvas.width, canvas.height); // change draw mode based on radio buttons
  ...
  var submitGuess = function submitGuess() {
    ...
    input.value = "";
    input.placeholder = "";
    if (!isMobile()) input.focus(); // display score and guesses left
  ...
  randomImg(images, imagesPath); // add event listeners for buttons
