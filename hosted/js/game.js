...
window.onload = function () {
  ...
  var newMon = document.querySelector('#new-mon');
  var modeForm = document.querySelector('#mode-form');
  ...
  var genModifiers = [document.querySelector('#gen1'), document.querySelector('#gen2'), document.querySelector('#gen3'), document.querySelector('#gen4'), document.querySelector('#gen5'), document.querySelector('#gen6'), document.querySelector('#gen7'), document.querySelector('#gen8'), document.querySelector('#gen9')];
  var formModifiers = {
  ...
  var scaleToFit = function scaleToFit(img) {
  ...
  var randomCropMobile = function randomCropMobile(img) {
    if (img.style.transform === 'scale(1)') img.style.transform = 'scale(5)';
  };
  ...
  var randomImg = function randomImg(imgsArr, imgsPath) {
    ...
    var img = isMobileRes ? monMobile : monDesktop;
    ...
    for (var i = 0; i < genModifiers.length; i++) {
    ...
    do {
      ...
      if (imgsArr[randGen][randMon].indexOf('Mega') > 0 && formModifiers['mega'].checked === false) formArr[0] = false;else formArr[0] = true;
      if (imgsArr[randGen][randMon].indexOf('Gigantamax') > 0 && formModifiers['gmax'].checked === false) formArr[1] = false;else formArr[1] = true;
      if (imgsArr[randGen][randMon].indexOf('Alola') > 0 && formModifiers['alola'].checked === false) formArr[2] = false;else formArr[2] = true;
      if (imgsArr[randGen][randMon].indexOf('Galar') > 0 && formModifiers['galar'].checked === false) formArr[3] = false;else formArr[3] = true;
      if (imgsArr[randGen][randMon].indexOf('Hisui') > 0 && formModifiers['hisui'].checked === false) formArr[4] = false;else formArr[4] = true;
      for (var _i = 0; _i < formArr.length; _i++) {
        if (formArr[_i] === false) {
          loop = true;
          break;
        } else {
    ...
    if (isMobileRes) img.style.visibility = 'hidden';

    // wait for image to load and draw it
    img.onload = function () {
      // clear canvas first (if applicable)
      if (!isMobileRes) ctx.clearRect(0, 0, canvas.width, canvas.height);
  ...
  var _loop = function _loop(i) {
    modeForm.elements[i].onchange = function () {
      // set game mode, reset score, force reroll
      mode = modeForm.elements[i].id.split('mode')[1];
      resetGame();
    };
  };
  for (var i = 0; i < modeForm.elements.length; i++) {
    _loop(i);
  }

  // add onchange handlers for checkboxes
  for (var _i2 = 0; _i2 < genModifiers.length; _i2++) {
    genModifiers[_i2].onchange = resetGame;
  }
  for (var key in formModifiers) {
    formModifiers[key].onchange = resetGame;
  }

  // return any pokemon name as a placeholder
  var getPlaceholder = function getPlaceholder() {
    var randGen = rand(IMAGE_ARR.length);
    var randMon = rand(IMAGE_ARR[randGen].length);
    var str = IMAGE_ARR[randGen][randMon];
